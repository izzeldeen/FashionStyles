...
namespace FashionStyles.DataAccess.Repositroy
{
    public class SP_Call : ISP_Call
    {
        ...
        public Tuple<IEnumerable<T1>, IEnumerable<T2>> List<T1, T2>(string procedurName, DynamicParameters param = null)
        {
            ...
            {
                sqlCon.Open();
                var reuslt = SqlMapper.QueryMultiple(sqlCon , procedurName, param, commandType: System.Data.CommandType.StoredProcedure);
                var item1 = reuslt.Read<T1>().ToList();
                var item2 = reuslt.Read<T2>().ToList();
        ...
        public T OneRecored<T>(string procedurName, DynamicParameters param = null)
        {
            using (SqlConnection sqlCon = new SqlConnection(ConnectionString))
            {
                sqlCon.Open();
